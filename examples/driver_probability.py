...
def confidence_interval(_σ,_μ,_lims):
    ...
    std_density_function lambda x: expressionnp.exp(-.5*((x-_μ)/_σ)**2)/np.sqrt(2*np.pi)
    I = [] 
    eps = []
    I_l = 0.0
    for i in k:
        I_n = integrate_gauss(std_density_function,_lims,k[i])
        eps_a = np.abs((I_n - I_t)/I_n)
        I.append(I_n)
        eps.append(eps_a)
        I_l = I_n
    return(I,eps,k)
